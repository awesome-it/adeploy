...
def helm_repo_pull(log, repo, name, version, dest):
    log.debug(
        f'Pulling chart "{colors.bold(name)}" from helm repo {colors.bold(repo)} to "{colors.bold(dest)}" ...'
    )
    return helm(
        log,
        ["pull", f"{repo}/{name}"]
...
def helm_prepare_chart(
    log: Logger, deployment: Deployment, chart_path: Path
) -> (str, str):
