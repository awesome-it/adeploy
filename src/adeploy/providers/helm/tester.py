...
class Tester(HelmProvider):
    ...
    def get_parser():
        parser = argparse.ArgumentParser(
            description="Helm v3 tester for k8s manifests", usage=argparse.SUPPRESS
        )
    ...
    def run(self):
        ...
        for deployment in self.load_deployments():
            ...
            try:
                ...
                last_update = (
                    f", last deployed {colors.bold(result.last_deployed)}"
                    if is_update
                    else ""
                )
                is_success = (
                    result.status == "pending-upgrade"
                    or result.status == "pending-install"
                )
                ...
                self.log.info(
                    f'... Testing raw manifests from "{colors.bold(manifest_path)}" (may fail) ...'
                )
                manifest_files = []
                ...
                try:
                    ...
                    for single_manifest_path in manifest_files:
                        ...
                        namespace = manifest_yaml.get("metadata", {}).get(
                            "namespace", False
                        )
                        ...
                        if "cluster" in kind.lower():
                            ...
                            result = kubectl_apply(
                                self.log, single_manifest_path, dry_run="server"
                            )
                        ...
                        parse_kubectrl_apply(
                            ...
                            deployment_ns=deployment.namespace,
                            prefix=2 * "...",
                        )
                    ...
                    keep_files.append(
                        cur_manifest_file
                    )  # Keep file for debugging purpose.
                    raise TestError(
                        f'Error when loading YAML file "{cur_manifest_file}": {e}'
                    )
                ...
                raise TestError(
                    f'Error in Helm deployment "{colors.blue(deployment)}": {e.stderr}'
                )
