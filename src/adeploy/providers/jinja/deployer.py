...
class Deployer(Provider):
    ...
    def get_parser():
        parser = argparse.ArgumentParser(
            description="Deploys k8s manifests written in Jinja",
    ...
    def run(self):
        ...
        for deployment in self.load_deployments():
            ...
            self.log.info(
                f'Applying manifests for deployment "{colors.blue(deployment)}" in "{deployment.manifests_dir}" ...'
            )
            ...
            for ext in ["yaml", "yml"]:
                files.extend(
                    glob.glob(f"{deployment.manifests_dir}/**/*.{ext}", recursive=True)
                )

            configmap_files = [
                file for file in files if self.manifest_is_configmap(file)
            ]
            non_configmap_files = [
                file for file in files if not self.manifest_is_configmap(file)
            ]
