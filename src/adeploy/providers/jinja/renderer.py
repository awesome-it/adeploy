...
class Renderer(Provider):
    ...
    def get_parser():
        parser = argparse.ArgumentParser(
            description="Jinja renderer for k8s manifests written in Jinja",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.jinja_pathes = [
            ...
            "..",
            self.templates_dir,
            str(Path(self.templates_dir).parent),
            str(Path(self.templates_dir).parent.parent),
        ]
        self.log.debug(
            f"Using Jinja file system loader with pathes: {', '.join(self.jinja_pathes)}"
        )
        self.env = jinja_env.create(
            self.jinja_pathes, log=self.log, templates_dir=self.templates_dir
        )
    ...
    def load_templates(self, extensions=None):
        ...
        self.log.debug(
            f'Scanning source dir with pattern "{self.templates_dir}/**/*.({"|".join(extensions)})" ...'
        )
        ...
        return self.templates_dir, sorted(
            [f.replace(f"{self.templates_dir}/", "") for f in files]
        )
    ...
    def render_template(
        self, deployment: Deployment, template_path: str, prefix: str = "..."
    ):
        ...
        try:
            ...
            raise RenderError(
                f'Jinja template error: Template "{e}" not found in "{template_path}"'
            )
            ...
            self.log.debug(f"Used Jinja variables: {json.dumps(values)}")
            raise RenderError(
                f'Jinja template error in "{colors.bold(template_path)}": {e}'
            )
        ...
        try:
            ...
            for index, doc in enumerate(documents):
                ...
                object_output_path = (
                    output_path.with_suffix(f".{index}.yml")
                    if len(documents) > 1
                    else output_path
                )
                ...
                with open(object_output_path, "w") as fd:
                    self.log.info(
                        f'{prefix} render {colors.bold(object_kind)} "{colors.bold(object_name)}" '
