        ...
        if "deploy" in self.args:
            ...
            for src_dir in self.args.src_dirs:
                ...
                if not os.path.isdir(src_dir):
                    self.log.warning(
                        colors.orange(f'"{src_dir}" is not a directory, skip')
                    )
                    num_warnings += 1
                    continue
                ...
                try:
                    ...
                    self.log.info(
                        colors.green_bold("Deploying ")
                        + colors.bold(src_dir)
                        + " in "
                        + colors.bold(self.args.build_dir)
                        + " using the provider "
                    ...
                    for secret in Secret.get_stored(build_dir, name):
                        ...
                        if deployment.skipped(self.args):
                            self.log.info(
                                f'... Secret "{colors.blue(secret.name)}" for '
                                f'deployment "{colors.blue(deployment)}" skipped by user filter.'
                            )
                            continue

                        # Skip cluster
                        if not deployer.verify_current_cluster_is_last_cluster(
                            deployment
                        ):
                            continue
                    ...
                    self.log.error(
                        colors.red(
                            f'Deployment failed in source directory "{src_dir}":'
                        )
                    )
