...
def kubectl_set_default_namespace(
    log: Logger, namespace: str
) -> subprocess.CompletedProcess:
    return kubectl(
        log, ["config", "set-context", "--current", f"--namespace={namespace}"]
    )
...
def kubectl_get_namespaces(log: Logger):
    return [
        ...
        for n in json.loads(
            kubectl(log, ["get", "namespace", "-o", "json"]).stdout
        ).get("items", [])
    ]


def kubectl_apply(
    log, manifest_path, namespace=None, dry_run=None, output=None
) -> subprocess.CompletedProcess:
...
def kubectl_create_secret(
    ...
    result = kubectl(
        log,
        ["create", "secret", type, name] + args + ["--dry-run=client", "-o", "json"],
        namespace,
    )
            ...
            if manifests:
                for item in manifests.get("items", [manifests]):
                    ...
                    if "/" in item.get("apiVersion"):
                        item_resource_name += (
                            "." + item.get("apiVersion").split("/")[0].lower()
                        )

                    if (
                        item_resource_name == resource
                        and metadata.get("name", None) == resource_name
                    ):
                        found = True
                        ...
                        if (
                            (namespace == "default" or namespace not in namespaces)
                            and deployment_ns
                            and deployment_ns != namespace
                        ):
                        ...
                        if "cluster" in item_resource_name.lower():
                            ...
                            raise TestError(
                                f"No namespace specified for "
                                f"resource {colors.bold(resource)}/{colors.bold(resource_name)} "
                            )
                        else:
                            namespace_description = (
                                f"namespace: {colors.bold(namespace or default_ns)} "
                            )
                ...
                if not found:
                    log.warning(
                        f"Could not find metadata for resource: {colors.bold(resource)}, "
                        f"name: {colors.bold(resource_name)}, ignore ..."
                    )

            log.info(
                f"{prefix} {namespace_description}"
