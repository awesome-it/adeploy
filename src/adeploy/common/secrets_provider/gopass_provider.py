...
class GopassSecretProvider(SecretsProvider):
    ...
    def __check_for_usable_gopass(cls) -> bool:
        if not GopassSecretProvider.__found_version:
            GopassSecretProvider.__found_version = (
                GopassSecretProvider.gopass_get_version()
            )
            if not GopassSecretProvider.__found_version:
    ...
    def _get_value(self, log: Logger = None) -> str:
        ...
        if result is None:
            raise InputError(
                f"Cannot find gopass value for {self.path} in repos {self.gopass_get_repos()}."
            )
    ...
    def gopass_get_version():
        try:
            result = subprocess.run(
                ["gopass", "version"], capture_output=True, text=True, check=True
            )
    ...
    def gopass_try_repos(self, log: Logger) -> subprocess.CompletedProcess:
        ...
        for repo in [Path(r) for r in GopassSecretProvider.gopass_get_repos()]:
            ...
            result = self.gopass_try(
                repo_path=secret_path, use_show=self.use_show, log=log
            )
    ...
    def gopass_try(
        ...
        cmd = (
            ["gopass", "show" if use_show else "cat"]
