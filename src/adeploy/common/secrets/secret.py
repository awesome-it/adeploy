...
class Secret(ABC):
    ...
    def clean_all(secrets, log, dry_run: bool = True):
        ...
        for d, secrets in deployments.items():
            if len(secrets) > 0:
                ...
                num_orphaned = 0
                log.debug(
                    f"... existing secrets: {colors.bold(', '.join(secrets_existing))}"
                )
                log.debug(
                    f"... created secrets: {colors.bold(', '.join(secrets_created))}"
                )
                for s in secrets_existing:
                    if s not in secrets_created:
                        ...
                        if not dry_run:
                            log.info(
                                f'... {colors.orange("delete")} orphaned secret "{colors.bold(d.name + "/" + s)}"'
                            )
                            kubectl_delete_secret(log, name=s, namespace=d.namespace)
                        else:
                            log.info(
                                f'... found orphaned secret "{colors.bold(d.name + "/" + s)}", '
    ...
    def __deprecated_get_value(
        ...
        if self.use_pass:
            ...
            return GopassSecretProvider(
                path=data, log=log, use_show=not self.use_gopass_cat
            ).get_value()
        # A plaintext secret
        warnings.warn(
    ...
    def get_path(self, build_dir):
        return (
            Secret.get_secret_dir(build_dir, self.deployment.name)
    ...
    def test(self, log: Logger):
        log.info(
            f'Testing secret "{colors.bold(self)}" for deployment "{colors.blue(self.deployment)}" ...'
        )

        # Test whether this secret is already deployed
        if self.exists(log):
            log.info(
                f"... secret already exists. "
    ...
    def deploy(self, log: Logger, recreate=False):
        if self.exists(log):
            if recreate:
                ...
                log.info(
                    f'... remove existing secret "{colors.bold(self)}" in order to re-create ...'
                )
            else:
        ...
        try:
            ...
            raise RenderError(
                f'Error while creating secret "{colors.bold(self.name)}": {e}\n{e.stderr.strip()}'
            )
