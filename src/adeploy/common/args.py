...
def parse(parser: argparse.ArgumentParser):
    ...
    if ("--help" in sys.argv or "-h" in sys.argv) and (
        "--provider" in sys.argv or "-p" in sys.argv
    ):
...
def setup_parser():
    parser = argparse.ArgumentParser(
        description="An awesome universal deployment tool for k8s",
        usage=colors.bold(
    ...
    parser.add_argument(
        "--adeploy-dir",
        dest="adeploy_dir",
        help="Configuration directory for adeploy",
        default=Path.home().joinpath(".adeploy"),
        metavar="adeploy_dir",
        type=Path,
    )
